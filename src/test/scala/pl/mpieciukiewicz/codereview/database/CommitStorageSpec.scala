...
class CommitStorageSpec extends FeatureSpec with GivenWhenThen with BeforeAndAfter {
  ...
  before {
    storage = new CommitStorage(new DocumentDataStorage(new DatabaseAccessor("jdbc:h2:mem:testdb", "sa", "sa"), new JsonUtil))
  ...
  feature("Properly storing commits data") {
    ...
    scenario("Storing and getting back correct commits") {
      ...
      val prototypeA = Commit(None, 1, DateTime.now(), "asdr23r", "Marcin", "Marcin", "first commit")
      val prototypeB = Commit(None, 2, DateTime.now(), "46fddh", "Marcin", "John", "second commit")
